define([
  ...
  p.render = function render() {
    ...
    if (state.status.visible && state.status.active) {
      var actualItem = state.items.itemList[state.items.selectedIndex],
        selectedText =
          actualItem === undefined
            ? undefined
            : state.text.textElements[actualItem.textId],
        selectedImage =
          actualItem === undefined
            ? undefined
            : state.image.imageElements[actualItem.imageId];
      ...
      var itemProps = {
        ...
        image: {
          image:
            selectedImage === undefined ? undefined : selectedImage.imagePath,
      ...
      if (state.items.listOpen) {
        this.listContainer = $(
          "<div id='" +
            this.elem.id +
            "_listBoxWrapper' class='listBoxContainer " +
            state.style.styleToApply +
            "'></div>"
        );
  ...
  function _closeListOnMouseDown(event) {
    ...
    if (
      this.openEventTimeStamp !== event.timeStamp &&
      !$.contains(this.listContainer.find(".ListView")[0], event.target) &&
      !this.elem.contains(event.target)
    ) {
  ...
  function _layoutArrowList(listPosition, ItemSize, listWidth, scaleFactor) {
    switch (listPosition) {
      ...
      case Enum.Position.left:
        ...
        this.arrow.css({
          "margin-top":
            (this.el.offset().top - this.arrow.offset().top) / scaleFactor +
            ItemSize.height / 2 -
            8,
  ...
  function _getBorderFromList(listContainer) {
    ...
    var topBorder = Utils.roundTo(
        parseFloat(dummyDiv.css("border-top-width")),
        0
      ),
      bottomBorder = Utils.roundTo(
        parseFloat(dummyDiv.css("border-bottom-width")),
        0
      ),
   ...
   */
  p.getLocation = function getLocation(
    listPosition,
    listPositionExt,
    listRect,
    pageHeight,
    ...
    switch (listPosition) {
      case Enum.Position.bottom:
        ...
        location.left =
          targetRect.left +
          (targetRect.width - listRect.width * scaleFactor) / 2;
        break;
      case Enum.Position.top:
        location.height += arrowSize;
        location.top =
          targetRect.top - (listRect.height + arrowSize) * scaleFactor;
        location.left =
          targetRect.left +
          (targetRect.width - listRect.width * scaleFactor) / 2;
        break;
      ...
      case Enum.Position.middle:
        location.top =
          targetRect.top +
          (targetRect.height - listRect.height * scaleFactor) / 2;
        location.left =
          targetRect.left +
          (targetRect.width - listRect.width * scaleFactor) / 2;
        break;
  ...
  function _ensureLimits(location, limits, scaleFactor, listPosition) {
    if (location.top < limits.top) {
      if (
        listPosition === Enum.Position.top ||
        listPosition === Enum.Position.middle
      ) {
    ...
    if (
      location.top + location.height * scaleFactor >
      limits.top + limits.height
    ) {
      location.height -=
        (location.top +
