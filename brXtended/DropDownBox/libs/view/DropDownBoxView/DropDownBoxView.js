...
define([
    ...
    */
    ...
    p.render = function render() {
        ...
        this.el.addClass(this.widget.internalData.styleClasses.widget + ' DropDownBoxView');
        ...
        if (state.status.visible && state.status.active) {
            ...
            if (state.items.listOpen) {
                this.listContainer = $(`<div id='${this.elem.id}_listBoxWrapper' class='${this.widget.internalData.styleClasses.listContainer} ${state.style.styleToApply} ${((state.style.additionalStyleToApply) ? ' ' + state.style.additionalStyleToApply : '')}'></div>`);
                this.arrow = $("<div class='arrow'></div>");
                this.listContainer.append(this.arrow);
                bodyEl.append(this.listContainer);
                var borderCorrection = _getBorderFromList(this.listContainer),
                    ...
                    limits = this.getLimits(state.items.listSettings.cropToParent, this.el, appView, bodyEl),
                    ...
                    pageHeight = bodyEl[0].clientHeight,
                ...
                bodyEl.on(BreaseEvent.MOUSE_DOWN, this.closeOnMouseDownBound);
                bodyEl.on('wheel', this.closeOnWheelBound);
    ...
    p.dispose = function dispose() {
        ...
        if (this.closeOnMouseDownBound !== undefined) {
            bodyEl.off(BreaseEvent.MOUSE_DOWN, this.closeOnMouseDownBound);
        }
        if (this.closeOnWheelBound !== undefined) {
            bodyEl.off('wheel', this.closeOnWheelBound);
        }
        if (this.closeOnMouseMoveBound !== undefined) {
            bodyEl.off(BreaseEvent.MOUSE_MOVE, this.closeOnMouseMoveBound);
        }
    };
    ...
    function _closeListOnMouseDown(event) {
        ...
        if (this.openEventTimeStamp !== event.timeStamp && !$.contains(this.listContainer.find('.ListView')[0], event.target) && !this.elem.contains(event.target)) {
            ...
            bodyEl.on(BreaseEvent.MOUSE_MOVE, this.closeOnMouseMoveBound);
        }
    }
