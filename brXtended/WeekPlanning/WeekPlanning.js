...
define([
  ...
  {
    core: { ContainerWidget: SuperClass },
    events : { BreaseEvent },
    enum: { Enum },
    decorators: { LanguageDependency },
    controller: { uiController },
    config: breaseConfig,
    services : {logger: loggerService, language},
  },
  Texts,
) {
  ...
  */
  ...
  p._initText = function () {
    var widget = this;
    widget.actualLang = language.getCurrentLanguage();
  ...
  function _textDeleteButtonInit() {
    if (this.settings.textDeleteButton !== undefined && this.settings.textDeleteButton !== "") {
      if (language.isKey(this.settings.textDeleteButton) === false) {
        this.setTextDeleteButton(this.settings.textDeleteButton);
      } else {
        this.setTextKeyDeleteButton(language.parseKey(this.settings.textDeleteButton), false);
      }
    }
  ...
  p.widgetAddedHandler = function (e) {
    ...
    if (addedWidget !== null) {
      ...
      var widgetState = uiController.getWidgetState(addedWidget.id);
      if (widgetState < Enum.WidgetState.INITIALIZED) {
        uiController.addWidgetOption(addedWidget.id, "position", "relative");
      } else {
  ...
  p.getBackgroundColorFromButton = function (button) {
    ...
    if(buttonWidgetId){
      var backgroundColor = uiController.callWidget(buttonWidgetId, "getBackColor");
  ...
  p.setTextDeleteButton = function (text, keepKey) {
    ...
    if (breaseConfig.editMode !== true) {
      if (language.isKey(this.settings.text) === true) {
        this.setTextKeyDeleteButton(language.parseKey(this.settings.text), false);
        this.langChangeHandler();
        return;
      }
    }
  ...
  p.langChangeHandler = function (e) {
    if (this.settings.textkeyDeleteButton) {
      this.setTextDeleteButton(language.getTextByKey(this.settings.textkeyDeleteButton), true);
    }
  };
  ...
  function _checkUniqueAction(widget) {
    ...
    childrenWidgetId = [];
    widget.el.find("[data-brease-widget]").each(function () {
      var action = uiController.callWidget(this.id, "getAction");
      childrenWidgetId.push(this.id);
      actions.push(action);
    });
    ...
    if(duplicates.length > 0){
      loggerService.log(999, Enum.EventLoggerCustomer.BUR, Enum.EventLoggerVerboseLevel.OFF, Enum.EventLoggerSeverity.ERROR, [], "Not unique actions, widgetId: " + childrenWidgetId[duplicates[0]]);
      widget.error = true;
    }
  ...
  function _checkUniqueArrayNum(widget) {
    var valueForArrays = [],
      childrenWidgetId = [];
    widget.el.find("[data-brease-widget]").each(function () {
      var valueForArray = uiController.callWidget(this.id, "getValueForArray");
      childrenWidgetId.push(this.id);
    ...
    if(duplicates.length > 0){
      loggerService.log(999, Enum.EventLoggerCustomer.BUR, Enum.EventLoggerVerboseLevel.OFF, Enum.EventLoggerSeverity.ERROR, [], "Not unique valueForArray, widgetId: " + childrenWidgetId[duplicates[0]]);
      widget.error = true;
    }
